...
if ($status->isFailed()) {
    $token = $payum->getTokenFactory()->createAuthorizeToken($gatewayName, $payment, 'http://localhost/refunded_backend.php');
    header("Location: " . $token->getAfterUrl());
    exit;
}

if (!$status->isCaptured()) {
    throw new LogicException('Only transactions that are registered and authorized can be cancelled.');
}
...
$gateway->execute(new \Payum\Core\Request\Refund($payment));
